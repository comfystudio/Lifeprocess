        ...
        @if(Auth::user()->is_gratuate=='n')
            <div class="panel-heading no-padding" role="tab">
              <h4 class="panel-title">
                <a id="open_intro_video" data-toggle="collapse" data-parent="#accordion" href="#collapseOne" aria-expanded="true" aria-controls="collapseOne">
                ...
                @elseif(Auth::user()->unlocked_module>0)
                        @if($module->id==Auth::user()->unlocked_module)
                            <div class="panel-heading no-padding {{$active}}" role="tab" id="{{ $counter }}">
                                ...
                                {!! link_to_route('view.feedback', 'Module Feedback ', ['module_id' => Crypt::encryptString($module->id),'exercise_id' => Crypt::encryptString($exercise->id), 'user_id' => $user->id], ['class' => 'module-feedback hide']) !!}
                                @else
                        ...
                        @if(trim($module->introduction_video) != "")
                                ...
                                <div class="col-md-12 col-sm-6 col-xs-6 left">
                                    <p>
                                        ...
                                        @endif
                                         ...
                                         @endif
                                            {{ trans('comman.introduction_video') }} - <a href="#" class="text-green openvideo" data-open="0" >Click here to show video</a>
                                    </p>
                                </div>
                                ...
                                <div class="col-md-12 col-sm-6 col-xs-6 right">
                                    ...
                                    @elseif($enable_next && $program->watch_video=='yes')
                                            ...
                                            <div data-vimeo-url="{{$module->introduction_video}}" class="loadvideo" id="modulevideo" data-vimeo-height="540px">
                                ...
                                <div class="col-md-7 col-sm-6 col-xs-6 right no-padding">
                                            <p class="no-padding">
                                               ...
                                               {{--  <a href="{{url('uploads/module/reading_materials/'.$module->reading_material)}}" onclick="load_module_video_orRead({{$module->id}},'material')" download class="text-green" target="_blank">{{trans('comman.download_reading_material')}}</a> --}}
                                                ...
                                                @endphp
                                                @else
                                                <a href="#" class="refresh" id='clickifwatch'>click here if video watched</a> |
...
<script type="text/javascript">
    ...
    jQuery('.openvideo').click(function(event) {
        /* Act on the event */
        if( jQuery('.openvideo').data('open') == 0){
            jQuery('.loadvideo').show();
            jQuery('.openvideo').html('Click here to close video');
            jQuery('.openvideo').data('open', 1);
        }else{
            jQuery('.loadvideo').hide();
            jQuery('.openvideo').html('Click here to show video');
            jQuery('.openvideo').data('open', 0);
        }
    });
...
//        jQuery('#collapseOne').css('display', '');
//        if(jQuery('#open_intro_video').attr('aria-expanded') == 'true'){
...
//    });
    ...
    jQuery('#clickifwatch').click(function(){
        var  previous_module_id=jQuery('#previous_module_id').val();
        var  next_module_id=jQuery('#next_module_id').val();
        var delay_btw_chapter_exercise=jQuery('#delay_btw_chapter_exercise').val();
        check_delay_between_modules(previous_module_id,next_module_id,delay_btw_chapter_exercise,'video');
