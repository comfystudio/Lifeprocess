    ...
    <div class="col-md-12">
        <div class="tabbable">
            <ul class="nav nav-tabs nav-tabs-highlight no-margin">
                <li class="active"><a href="#all-coach" data-toggle="tab" aria-expanded="true">Coachs</a></li>
                <li class="">
                ...
                <div class="tab-pane fade fade active in" id="all-coach">
                    <div class="tabbable nav-tabs-vertical nav-tabs-left">
                        <ul class="nav nav-tabs nav-tabs-highlight">
                            ...
                            @endif
                            {{-- @if($coach->isOnline())
                                @php $online='user is online!!';@endphp --}}
                            ...
                            @endif --}}
                                @if($count++ == 0)
                                    <li class="active">
                                    {!! Html::decode(link_to_route('messages.admindata', $online." ".$coach['name'], ['role'=>'coach','id' =>Crypt::encryptString($coach['id'])])) !!}
                                    </li>
                                @else
                                    <li class="">
                                    {!! Html::decode(link_to_route('messages.admindata', $online." ".$coach['name'], ['role'=>'coach','id' =>Crypt::encryptString($coach['id'])])) !!}
                        ...
                        @if(isset($all_coach) && count($all_coach) > 0 )
                            {{-- @php $all_coach[0] = $all_coach->first(); @endphp --}}
                            <div class="tab-content no-padding">
                                <div class="panel panel-default no-border">
                                    <div class="panel-heading no-border">
                                        <h5 class="panel-title">
                                            Your conversation with {{$all_coach[0]['name']}}


                                        </h5>
                                    </div>
                                    <div class="panel-body no-padding">
                                    <div class="tab-pane fade fade active in" id="{{$all_coach[0]['id']}}">

                                        <div class="message-listing">
                                        <div class="row no-margin left">
                                            <div class="msg" id="chat" style="">
                                            @foreach($messages as $message)
                                                @if($message->receive_user_id == $all_coach[0]['id'] || $message->create_user_id == $all_coach[0]['id'])
                                                    {{-- <li class="media date-step content-divider">
                                                        <span></span>
                                                    </li> --}}
                                                    @if($message->create_user_id != Auth::id())
                                                        <div class="row no-margin left">
                                                            @if(isset($all_coach[0]['image']) && !empty($all_coach[0]['image']))
                                                                {{Html::image(AppHelper::path('uploads/user/')->size('36x36')->getImageUrl($all_coach[0]['image']),'User Photo',array("class"=>"pull-left logo"))}}
                                                            @else
                                        ...
                                        <div class="row new-msg no-margin">
                                            ...
                                            {!! Form::open(array('route' =>  ['messages.save', 'id' => Crypt::encryptString($all_coach[0]['id'])],'class'=>'form-horizontal send-msg','role'=>"form",'files' => true)) !!}
                                            <div class="form-group no-margin {{ $errors->has('messages') ? 'has-error' : ''}}">
                                                <h4>Send a reply</h4>
                                                <div class="emoji-picker-container">
                                                    {!! Form::textarea('messages', null, ['class' => 'form-control','placeholder' => trans("comman.message"),'rows' => '5','data-emojiable' => 'true','id'=>'messages']) !!}
