...
<div class="sidebar sidebar-main sidebar-default">
    <div class="sidebar-content">
        ...
        <div class="sidebar-category sidebar-category-visible">
            <div class="category-title h6 " @if(isset($agent_theme) && !empty($agent_theme['colour_3'])) style="background: {{$agent_theme['colour_3']}}"@else style="background:#E7F4FD;" @endif>
                ...
                <ul class="navigation navigation-main navigation-accordion">
                    <!-- Main -->
                        ...
                        @if(Auth::user()->hasAccess('messages.view') && Auth::user()->user_type != 'coach')
                            ...
                            @endif
                        @endif
                        {{--@if(Auth::user()->hasAccess('messages.view') && Auth::user()->user_type == 'read-only-coach')--}}
                            {{--<li class="{{ (Request::segment(1)=='messages' ? 'active' : '' ) }}">--}}
                                {{--<a href="{{ route('messages.getrole', ['role'=>'read-only-coach']) }}" class="badge1" {{ $unread_counter != '0' ? 'data-badge='.$unread_counter.'' : '' }}><i class="fa fa-wechat"></i>Messages</a>--}}
                            {{--</li>--}}
                        {{--@endif--}}
                        ...
                        @if(Auth::user()->hasAnyAccess(['report.coach_report','report.financial_report.view','report.signup_report.view','report.refer_friend_report.view']))
                            @if(Auth::user()->user_type != 'read-only-coach' && !isset($agent_theme))
                                <li>
                                    <a href="#">
                        ...
                        @if(Auth::user()->user_type=='agent')
                            <li class="{{ (Request::segment(1)=='edit-theme' ? 'active' : '' ) }}">
                                <a href="{{ url('agent/edit-theme') }}">
                                    <i class="fa fa-paint-brush"> </i><span class="font-semibold text-black">Edit Theme</span>
                                </a>
                            </li>
                        @endif
